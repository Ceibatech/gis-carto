...
export function buildCeibaProductionMetrics(rows = [], now = new Date()) {
  ...
  for (const row of normalizedRows) {
    ...
    const quantity = Number(row?.cartonsCount ?? row?.cartonCount ?? row?.quantity ?? row?.totalCartons ?? 0);
    const hasExplicitQuantity = Number.isFinite(quantity) && quantity > 0;
    ...
    if (hasExplicitQuantity) {
      const contribution = Number(quantity) || 0;
      current.total += contribution;
      if (createdDate >= startOfToday) current.today += contribution;
      if (createdDate >= startOfWeek) current.week += contribution;
      if (createdDate >= startOfMonth) current.month += contribution;
    } else if (cartonId) {
  ...
  return {
    ...
    weekProduction,
    monthProduction,
    totalProduction: byAgentList.reduce((sum, item) => sum + item.total, 0),
