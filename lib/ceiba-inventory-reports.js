...
export function buildCeibaProductionMetrics(rows = [], now = new Date()) {
  ...
  function keyForAgentPeriod(agent, createdDate, bucketStart) {
    return `${agent}::${createdDate.toISOString().slice(0, 10)}::${bucketStart.toISOString().slice(0, 10)}`;
  }
  ...
  for (const row of normalizedRows) {
    ...
    if (cartonId) allTotals.add(`${agent}::${cartonId}`);

    const current = byAgent.get(agent) ?? { agent, today: new Set(), week: new Set(), month: new Set(), total: new Set() };
    if (cartonId) {
      current.total.add(cartonId);
      if (createdDate >= startOfToday) current.today.add(cartonId);
      if (createdDate >= startOfWeek) current.week.add(cartonId);
      if (createdDate >= startOfMonth) current.month.add(cartonId);
    }
    byAgent.set(agent, current);
  }

  const byAgentList = Array.from(byAgent.values())
    .sort((a, b) => b.total.size - a.total.size || a.agent.localeCompare(b.agent, "fr"))
    .map((item) => ({
      agent: item.agent,
      today: item.today.size,
      week: item.week.size,
      month: item.month.size,
      total: item.total.size,
    }));

  const todayProduction = byAgentList.reduce((sum, item) => sum + item.today, 0);
  const weekProduction = byAgentList.reduce((sum, item) => sum + item.week, 0);
  const monthProduction = byAgentList.reduce((sum, item) => sum + item.month, 0);

  return {
    todayProduction,
    weekProduction,
    monthProduction,
    totalProduction: allTotals.size,
